#!/usr/bin/env python3
"""
Fig 1: CAST technical roadmap (main paper) - Plant-ISEA3H version.
Adapted to output to figures/case4_plant/.
Run from project root: python scripts/Plant/plot/fig1_cast_technical_roadmap_plant.py
"""
...
# Project root: script lives in scripts/Plant/plot/
...
def main():
    ...
    ax.text(CX, iy + 0.28, 'Plant-ISEA3H Environmental Variables',
            ha='center', fontsize=14, fontweight='bold', color=BK)
    ax.text(CX, iy - 0.02, '13 core variables  ·  3648 grids',
    ...
    ax.text(CX + 0.2, mid_v, 'VIF filtering (Max < 7.5)', ha='left', fontsize=8.5, color=GY)

    box(ax, CX, s1y, W, s1h, lw=1.2)
    ax.text(LX + 0.35, s1y + 0.58, 'Stage 1', ha='left', fontsize=13, fontweight='bold', color=BK)
    ax.text(LX + 1.75, s1y + 0.58, 'Causal Structure Learning', ha='left', fontsize=13, color=BK)
    ax.text(LX + 0.45, s1y + 0.08, 'Bootstrap Hill-Climbing', ha='left', fontsize=10, color=GY)
    ax.text(LX + 0.45, s1y - 0.20, 'B = 100  ·  BIC-Gaussian', ha='left', fontsize=10, color=GY)
    ...
    ax.text(LX + 0.35, s2y + 0.98, 'Stage 2', ha='left', fontsize=13, fontweight='bold', color=BK)
    ax.text(LX + 1.75, s2y + 0.98, 'Causal Effect Estimation', ha='left', fontsize=13, color=BK)
    ate_cx  = CX - W / 4
    cate_cx = CX + W / 4
    ax.text(ate_cx, s2y + 0.48, 'ATE (Global)', ha='center', fontsize=11, fontweight='bold', color=BK)
    ax.text(ate_cx, s2y + 0.14, 'Double Machine Learning', ha='center', fontsize=8, color=GY)
    draw_forest(ax, ate_cx, s2y - 0.38, s=0.72)
    ax.plot([CX, CX], [s2y - 0.88, s2y + 0.68], color=LG, lw=0.5, ls=':', zorder=3)
    ax.text(cate_cx, s2y + 0.48, 'CATE (Spatial Proxy)', ha='center', fontsize=11, fontweight='bold', color=BK)
    ax.text(cate_cx, s2y + 0.14, 'Spatial Projection', ha='center', fontsize=8, color=GY)
    draw_heatmap(ax, cate_cx, s2y - 0.42, s=0.88)
    ax.text(CX, s2y - 1.05, 'Global ATE per variable  +  Spatial proxy per location',
            ha='center', fontsize=9, color=BK)

    arr(ax, CX, s2y - s2h / 2 - GAP, CX, s3y + s3h / 2 + GAP)

    box(ax, CX, s3y, W, s3h, lw=1.2)
    ax.text(LX + 0.35, s3y + 1.33, 'Stage 3', ha='left', fontsize=13, fontweight='bold', color=BK)
    ax.text(LX + 1.75, s3y + 1.33, 'Adaptive Screening  +  CI-MLP (27 species)', ha='left', fontsize=13, color=BK)
    crit_y  = s3y + 0.58
    crit_xs = [LX + 0.95, LX + 2.1, LX + 3.25]
    crit_colors = [C_TEAL, C_RED, C_GREEN]
    for i, (lbl, col) in enumerate(zip(
            ['DAG\nOut-degree', 'ATE\nEffect size', 'RF\nImportance'], crit_colors)):
        ...
        ax.plot(crit_xs[i], crit_y + 0.35, 's', color=col, ms=4, zorder=8, clip_on=False)
    merge_cx = crit_xs[1]
    for bx in crit_xs:
    ...
    ax.text(LX + 0.6, s3y - 0.92, 'ATE-weighted features', ha='left', fontsize=7.5, color=GY)
    ax.text(LX + 0.6, s3y - 1.18, '+ DAG interaction features', ha='left', fontsize=7.5, color=GY)
    nn_cx = CX + W / 2 - 1.35
    arr(ax, merge_cx + 1.65, score_y, nn_cx - 1.05, score_y, lw=0.8)
    draw_nn(ax, nn_cx, s3y - 0.30, s=0.95)
    ax.text(nn_cx, s3y - 1.08, 'CI-MLP', ha='center', fontsize=11, fontweight='bold', color=BK)
    ax.text(nn_cx, s3y - 1.36, 'Focal loss  ·  3 seeds', ha='center', fontsize=7, color=GY)

    fig.savefig(os.path.join(FIG_DIR, 'fig1_cast_technical_roadmap_plant.png'), dpi=1200,
                bbox_inches='tight', facecolor='white', edgecolor='none')
    print('[OK] fig1_cast_technical_roadmap_plant.png')
    fig.savefig(os.path.join(FIG_DIR, 'fig1_cast_technical_roadmap_plant.svg'), format='svg',
                bbox_inches='tight', facecolor='white', edgecolor='none')
    print('[OK] fig1_cast_technical_roadmap_plant.svg')
    plt.close()
