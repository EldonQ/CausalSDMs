...
bioc_info <- data.frame(
  ...
  name = c(
    "Ann Mean Temp", "Mean Diurnal Range", "Isothermality",
...
for (ssp in ssp_scenarios) {
  ...
  bioc_file <- file.path(
    "E:/WorldClim/Future", ssp,
    paste0(
      "wc2.1_30s_bioc_BCC-CSM2-MR_",
...
mdl_files <- c(
  Maxnet = file.path(models_path, "maxnet.rds"),
...
build_future_env <- function(bioc_stack) {
  ...
  temp_ann_mean <- bioc_stack[["bio01"]] / 10
  names(temp_ann_mean) <- "temp_ann_mean"
  prec_ann_sum <- bioc_stack[["bio12"]]
  names(prec_ann_sum) <- "prec_ann_sum"
  static_list <- list()
...
if (length(all_summaries) > 0) {
  ...
  p_trend <- ggplot(trends, aes(x = scenario, y = mean, group = model, color = model)) +
    ...
    geom_point(size = 1.4) +
    labs(
      title = "Habitat Suitability Trend across SSP Scenarios",
