...
#          output/15b_causal_retraining/models/{maxnet,rf}_causal.rds
...
# 加载必要的包
packages <- c(
  "tidyverse", "maxnet", "randomForest", "pROC",
...
if (file.exists(ate_path)) {
  ...
  ate_sig <- ate_df %>%
    ...
    dplyr::select(variable, coef, p_value)
  cat("\n  - ATE显著变量:\n")
...
dag_summary <- node_metrics %>%
  head(15) %>%
  mutate(source = "DAG")
imp_summary_top <- imp_summary %>%
  head(15) %>%
  mutate(source = "Importance")
ate_summary_top <- if (nrow(ate_sig) > 0) {
  ate_sig %>%
    head(10) %>%
    mutate(source = "ATE")
} else {
...
selection_summary <- bind_rows(
  dag_summary %>% dplyr::select(variable = from, metric = out_degree, source),
  imp_summary_top %>% dplyr::select(variable, metric = mean_importance, source),
  if (nrow(ate_summary_top) > 0) ate_summary_top %>% dplyr::select(variable, metric = coef, source) else NULL
)
...
model_maxnet <- maxnet::maxnet(
  p = train_maxnet$presence,
...
model_rf <- randomForest::randomForest(presence ~ .,
  data = train_rf,
...
for (mn in names(models_causal)) {
  pred_prob <- predict_causal(models_causal[[mn]], mn, X_test, NULL)
  ...
  cat(sprintf(
    "  %8s: AUC=%.3f, TSS=%.3f, Sens=%.3f, Spec=%.3f\n",
...
comparison <- bind_rows(
  eval_full %>% mutate(model_type = "Full (47 vars)") %>% dplyr::select(model, model_type, n_vars, auc, tss, sensitivity, specificity),
  eval_causal %>% mutate(model_type = paste0("Causal (", length(core_drivers), " vars)")) %>% dplyr::select(model, model_type, n_vars, auc, tss, sensitivity, specificity)
)

# 计算性能保留率
retention <- eval_causal %>%
  left_join(eval_full %>% dplyr::select(model, auc_full = auc, tss_full = tss), by = "model") %>%
  mutate(
...
cat("\n  === 简化模型性能保留率 ===\n")
print(retention %>% dplyr::select(model, n_vars, auc_retention, tss_retention, var_reduction))
...
p_auc <- ggplot(comparison, aes(x = model, y = auc, fill = model_type)) +
  ...
  labs(
    title = "AUC Comparison: Full vs. Causal Models",
    x = "Algorithm", y = "AUC"
  ) +
  ylim(0, 1) +
...
p_tss <- ggplot(comparison, aes(x = model, y = tss, fill = model_type)) +
  geom_col(position = position_dodge(0.8), width = 0.7) +
  geom_text(aes(label = sprintf("%.3f", tss)),
    position = position_dodge(0.8),
    vjust = -0.5, size = 2.5, family = "Arial"
  ) +
  scale_fill_manual(values = color_values, name = "Model Type") +
  labs(
    title = "TSS Comparison: Full vs. Causal Models",
    x = "Algorithm", y = "TSS"
  ) +
  ylim(0, 1) +
  viz_theme_nature(base_size = 8, title_size = 9) +
...
p_retention <- ggplot(retention, aes(x = var_reduction, y = auc_retention)) +
  ...
  labs(
    title = "Variable Reduction vs. Performance Retention",
