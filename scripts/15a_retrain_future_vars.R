...
get_band_mean <- function(br) {
  raster::calc(br, fun = function(x) {
    if (all(is.na(x))) NA else mean(x, na.rm = TRUE)
  })
}
get_band_sum <- function(br) {
  raster::calc(br, fun = function(x) {
...
ann_temp_mean <- (ann_tmax + ann_tmin) / 2
names(ann_temp_mean) <- "temp_ann_mean"
ann_prec_sum <- get_band_sum(pr_br)
names(ann_prec_sum) <- "prec_ann_sum"
