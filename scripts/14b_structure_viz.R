...
# 备注: 图件英文标注、Arial、≥600dpi、PNG+SVG，矢量优先
# ==============================================================================
...
# 加载包
pkgs <- c(
  "tidyverse", "bnlearn", "igraph", "ggraph", "tidygraph",
...
plot_network <- function(tg, title_text, file_stub, layout = "kk") {
  ...
  ggsave(paste0("figures/14_causal/", file_stub, ".png"), p,
    width = 14, height = 10, units = "in", dpi = 600, bg = "transparent"
  )
...
p_core <- ggraph::ggraph(tg_core, layout = "kk") +
  ...
  labs(
    title = paste0("Core Causal Pathways (Top ", topN, ")"),
    subtitle = "Edge width/color = bootstrap strength"
  ) +
  theme_void(base_family = "Arial")
ggsave("figures/14_causal/dag_core_topN.png", p_core,
  width = 16, height = 12, units = "in", dpi = 1200, bg = "transparent"
)
...
edges_cross <- edges_top %>%
  dplyr::left_join(var_groups, by = c("from" = "name")) %>%
  dplyr::rename(from_group = group_label) %>%
  dplyr::left_join(var_groups, by = c("to" = "name")) %>%
  dplyr::rename(to_group = group_label) %>%
...
# 5) 边强度矩阵
cat("绘制: 边强度矩阵 ...\n")
edges_mat <- edges_strength %>%
  dplyr::filter(strength >= min_strength_matrix) %>%
...
adj_long <- reshape2::melt(adj) %>%
  dplyr::filter(value > 0) %>%
  dplyr::rename(from = Var1, to = Var2, strength = value)
...
ggsave("figures/14_causal/dag_strength_matrix.png", p_mat,
  width = 12, height = 11, units = "in", dpi = 1200, bg = "transparent"
)
...
if (!is.null(pc_fit)) {
  ...
  ggsave("figures/14_causal/dag_pc_layout.png", p_pc,
    width = 10, height = 7.5, units = "in", dpi = 1200, bg = "transparent"
  )
