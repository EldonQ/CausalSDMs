...
# ── Configurable ─────────────────────────────────────────
SKIP_VIF <- TRUE         # TRUE = skip iterative VIF, keep all expert-filtered vars
VIF_THRESHOLD <- 10       # Standard threshold, some use 5 (ignored when SKIP_VIF = TRUE)
...
cat(sprintf("  SKIP_VIF = %s  |  VIF threshold = %d\n", SKIP_VIF, VIF_THRESHOLD))
cat("================================================================\n")
...
# ============================================================
# STEP 2: Iterative VIF elimination (or skip)
# ============================================================
cat(sprintf("\nStep 2: VIF screening (SKIP_VIF = %s)...\n", SKIP_VIF))
...
if (SKIP_VIF) {
    cat("  ⏩ Skipping iterative VIF elimination.\n")
    final_vars <- remaining
    cat(sprintf("  Using all %d expert-filtered variables.\n", length(final_vars)))
} else {
...
# Print final VIF values (only when VIF was run)
if (!SKIP_VIF) {
    cat("\n  Final VIF values:\n")
...
# Save VIF screening log (empty if skipped)
...
classify_var <- function(v) {
    ...
    if (grepl("tree|bare|nontree|landcover|vcf|igbp", v, ignore.case = TRUE)) {
...
n_orig <- length(numeric_env)
if (SKIP_VIF) {
    before_after <- data.frame(
        stage = factor(c(sprintf("Original (%d)", n_orig), "Expert filter"),
            levels = c(sprintf("Original (%d)", n_orig), "Expert filter")
        ),
        n_vars = c(n_orig, length(remaining))
    )
    p_ba <- ggplot(before_after, aes(x = stage, y = n_vars, fill = stage)) +
        geom_col(width = 0.6) +
        geom_text(aes(label = n_vars), vjust = -0.5, fontface = "bold", size = 4) +
        scale_fill_manual(values = c("#E74C3C", "#F39C12"), guide = "none") +
        theme_minimal(base_size = 12) +
        labs(
            title = "Variable Screening Pipeline (VIF skipped)",
            subtitle = sprintf(
                "%d variables retained (no VIF elimination)",
                length(remaining)
            ),
    ...
    before_after <- data.frame(
        stage = factor(c(sprintf("Original (%d)", n_orig), "Expert filter", "VIF screened"),
            levels = c(sprintf("Original (%d)", n_orig), "Expert filter", "VIF screened")
        ),
        n_vars = c(n_orig, length(remaining), length(final_vars))
    )
    p_ba <- ggplot(before_after, aes(x = stage, y = n_vars, fill = stage)) +
        geom_col(width = 0.6) +
        geom_text(aes(label = n_vars), vjust = -0.5, fontface = "bold", size = 4) +
        ...
        labs(
            ...
            subtitle = sprintf(
                "%d → %d → %d variables (%.0f%% reduction)",
                n_orig, length(remaining), length(final_vars),
                100 * (1 - length(final_vars) / n_orig)
            ),
            x = NULL, y = "Number of variables"
        )
}
...
cat("\n================================================================\n")
if (SKIP_VIF) {
    cat("=== Variable Screening Complete (VIF Skipped) ===\n")
} else {
...
if (SKIP_VIF) {
    cat(sprintf("  VIF removed:          0 (skipped)\n"))
} else {
