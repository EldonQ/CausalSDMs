################################################################################
# Fig S5 (Eco): Supplementary — DAG Structure & CATE Ecological Impacts
#
# (a) Causal graph complexity (DAG density) distribution across Taxonomic Families
# (b) Diverging bar chart of significant ATE directions per Family
# (c) Ridge plot: Distribution of significant causal effects for top variables
#
# Requires: all_results_v3.csv, all_ate_results_v3.csv, CAST_Species_Summary.csv
...
# 2. Prepare Meta Data
# Merge family info into DAG density and ATE results
dag_data <- d %>%
...
# ══════════════════════════════════════════════════════════════
# Panel (a): DAG Density by Taxonomic Family (Boxplot + Jitter)
# ══════════════════════════════════════════════════════════════
pa <- ggplot(dag_data, aes(x = family, y = dag_density, fill = family)) +
    geom_boxplot(alpha = 0.6, width = 0.5, outlier.shape = NA) +
    geom_jitter(width = 0.15, height = 0, alpha = 0.7, size = 1.8, color = "grey20") +
    scale_fill_brewer(palette = "Set3", guide = "none") +
    coord_flip() +
    labs(
        title = "(a) Causal network complexity by family",
...
# ══════════════════════════════════════════════════════════════
# Panel (b): Diverging Bar Chart of ATE Directions
# ══════════════════════════════════════════════════════════════
ate_fam <- ate %>%
...
# Ensure all families from panel (a) are present for alignment
...
pb <- ggplot(fam_counts, aes(x = family, y = count_val, fill = direction)) +
    ...
    theme_pub() +
    theme(
        axis.text.y = element_blank(), # visually link with panel a
        legend.position = "bottom"
    )

# ══════════════════════════════════════════════════════════════
# Panel (c): Ridge Plot of Effect Sizes for Top Variables
# ══════════════════════════════════════════════════════════════
# Find the top N variables that have the most significant effects across all species
top_vars <- ate %>%
    filter(significant == TRUE) %>%
    group_by(variable) %>%
    summarise(n = n()) %>%
    arrange(desc(n)) %>%
    slice_head(n = 7) %>% # Top 7 variables for better density representation
    pull(variable)
...
pc <- ggplot(ate_ridge, aes(x = coef, y = variable, fill = after_stat(x))) +
    ...
    labs(
        title = "(c) Causal effect size (ATE) distribution for core variables",
        subtitle = sprintf("Top %d most frequently significant variables across %d species", length(top_vars), length(unique(d$species))),
...
# Combine Layout
# ══════════════════════════════════════════════════════════════
# Ensure exact alignment of A and B plots
design <- "
AABB
CCCC
"
...
figS5 <- pa + pb + pc +
    ...
    plot_annotation(
        title = "Fig S5 (Eco) Causal Graph Complexity & Ecological Effect Heterogeneity",
