...
header = [["Model", "AUC (mean±sd)", "AUC range", "TSS (mean±sd)", "TSS range",
           "n_vars", "n_features", "ΔAUC vs MLP", "Win/Tie/Loss", "Wilcoxon p"]]

col_w = [1.2, 1.5, 1.3, 1.5, 1.3, 0.7, 0.9, 1.1, 1.1, 1.0]
...
for _, row in summary_df.iterrows():
    ...
    auc_range = f"[{row['auc_min']:.4f}, {row['auc_max']:.4f}]"
    tss_str = f"{row['tss_mean']:.4f}±{row['tss_sd']:.4f}"
    tss_range = f"[{row['tss_min']:.3f}, {row['tss_max']:.3f}]"
    n_vars = f"{row['n_vars']:.0f}"
    n_feat = f"{row['n_features']:.0f}"
    ...
    if m == "CAST":
        ...
        if len(cr) > 0:
            # delta vs CAST, but we want vs MLP
            # Compute directly
    ...
    # Bold mask: bold the best AUC and TSS
    bold = [False] * 10
    ...
    data_rows.append({
        "cells": [f"  {model_display[m]}", auc_str, auc_range,
                  tss_str, tss_range, n_vars, n_feat,
...
draw_booktabs_table(
    ...
    title=f"Table 2  Model performance comparison across {n_species} mammal species (EcoISEA3H, China)",
    footnote=(
        f"Notes: n = {n_species} species. AUC = Area Under ROC Curve; TSS = True Skill Statistic. "
        "Bold = best across all models. Win/Tie/Loss = species-level comparison vs CAST (AUC). "
        "Wilcoxon p = paired signed-rank test (CAST vs. competitor). "
        "ΔAUC vs MLP = mean AUC difference relative to Flat MLP baseline. "
        "CAST uses causal DAG-guided interaction features; MLP+ATE uses ATE-weighted inputs only."
    )
)
