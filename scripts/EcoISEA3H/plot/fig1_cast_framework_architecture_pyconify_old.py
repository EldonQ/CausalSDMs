...
try:
    ...
    try:
        ...
        print('[pyconify] found but Pillow not available — fallback to FA')
except Exception:
...
def _pyconify_get_png_bytes(icon: str, color: str = None, size: int = 64):
    ...
    # 尝试常见 API
    try_fns = []
    for nm in ("png", "get_png", "get", "icon", "fetch"):
        if hasattr(pyconify, nm):
            try_fns.append(getattr(pyconify, nm))

    last_err = None
    for fn in try_fns:
        try:
            # 尽量用 kwargs，避免位置参数版本差异
            kwargs = {}
            # icon/name 参数
            for k in ("icon", "name", "id"):
                kwargs[k] = icon
                break
            # size 参数
            for k in ("size", "height", "width"):
                kwargs[k] = size
                break
            # color 参数（可选）
            if color is not None:
                for k in ("color", "hex", "fill"):
                    kwargs[k] = color
                    break
            ...
            if isinstance(out, (bytes, bytearray)):
                _ICON_CACHE[key] = bytes(out)
                return _ICON_CACHE[key]
            # 有的实现可能返回 dict 或 Response-like
            if hasattr(out, "content"):
                _ICON_CACHE[key] = bytes(out.content)
                return _ICON_CACHE[key]
    ...
    if last_err is not None:
        print(f"[pyconify] failed to fetch {icon}: {last_err}")
    return None
...
def iconify_icon(ax, x, y, icon: str, size_px: int = 26, color: str = BK, alpha: float = 1.0):
    ...
    try:
        ...
        zoom = size_px / max(im.size)
        oi = OffsetImage(np.asarray(im), zoom=zoom, resample=True)
        oi.set_alpha(alpha)
...
def draw_input_column(ax, cx, cy_top, s=1.0):
    ...
    items = [
        ...
        ('\uf0c2', 'Climate',    C_BLUE,   'wave',     'mdi:weather-cloudy'),
    ...
    for i, (fa_code, name, col, fallback, iconify_id) in enumerate(items):
        y = cy_top - i * gap
        drawn = iconify_icon(ax, cx, y, iconify_id, size_px=int(20 * s), color=col, alpha=0.95)
        if not drawn:
...
def main():
    ...
    for ext in ['png', 'svg']:
        fp = os.path.join(FIG_DIR,
                          f'fig1_cast_framework_architecture_pyconify.{ext}')
