################################################################################
# Fig 2 (Eco): CAST single-species causal discovery showcase (4-panel)
# (a) DAG network  (b) ATE forest plot  (c) Screening scores  (d) SubDAG focus
#
# Adapted for EcoISEA3H workflow.
# Data source: all_*_v3.csv (case2_eco outputs)
# Run: setwd("E:/CausalSDMs"); source("scripts/EcoISEA3H/plot/fig2_cast_single_species_showcase_eco.R")
################################################################################

rm(list = ls())
setwd("E:/CausalSDMs")

target_species <- "Ovis_ammon"
...
results <- read.csv("output/case2_eco/all_results_v3.csv", stringsAsFactors = FALSE) %>%
    filter(region == target_region, species == target_species)
screening <- read.csv("output/case2_eco/all_screening_v3.csv", stringsAsFactors = FALSE) %>%
    filter(region == target_region, species == target_species)
ate_data <- read.csv("output/case2_eco/all_ate_results_v3.csv", stringsAsFactors = FALSE) %>%
    filter(region == target_region, species == target_species) %>%
    mutate(
...
dag_edges <- read.csv("output/case2_eco/all_dag_edges_v3.csv", stringsAsFactors = FALSE) %>%
    filter(region == target_region, species == target_species)

# Role info
roles_file <- "output/case2_eco/all_role_info_v3.csv"
...
if (nrow(dag_edges) > 0) {
    ...
    E(g)$strength <- as.numeric(dag_edges$strength)
    ...
    pa <- ggraph(g, layout = "fr") +
        ...
        labs(subtitle = sprintf(
            "%d edges | density %.2f",
            nrow(dag_edges), ifelse(!is.na(dag_density_val), as.numeric(dag_density_val), 0)
        )) +
        theme_pub() +
...
if (nrow(screening) > 0 && "score_dag" %in% names(screening)) {
    ...
    screen_long <- screening %>%
        ...
        mutate(
            w_dag = as.numeric(w_dag * score_dag),
            w_ate = as.numeric(w_ate * score_ate),
            w_imp = as.numeric(w_imp * score_imp)
        ) %>%
...
if (nrow(dag_edges) > 0 && length(cast_vars) > 0) {
    ...
    if (nrow(sub_edges) > 0) {
        ...
        E(gs)$strength <- as.numeric(sub_edges$strength)
        pd <- ggraph(gs, layout = "fr") +
            ...
            geom_node_text(aes(label = name), repel = TRUE, size = 3.2, fontface = "bold", color = "black") +
...
fig2 <- (pa | pb) / (pc | pd) +
    plot_annotation(
        title = sprintf("Fig 2 (Eco) CAST causal discovery: %s [%s]", gsub("_", " ", target_species), target_region),
        theme = theme(plot.title = element_text(face = "bold", size = 14, hjust = 0.5))
    )

ggsave(file.path(fig_dir, "fig2_cast_single_species_showcase_eco.png"),
    fig2,
    width = 14, height = 10, dpi = 300, bg = "white"
)
cat("Saved fig2_cast_single_species_showcase_eco.png to", fig_dir, "\n")
