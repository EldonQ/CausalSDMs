...
if (file.exists(override_path)) {
  ...
  sel_map <- whitelist_df[whitelist_df$variable %in% selected_vars_cor, c("variable", "category", "file", "band", "heatmap_group", "plot_label")]
  ...
  # 确保匹配顺序
  match_idx <- match(colnames(plot_cor_matrix), whitelist_df$variable)
  new_labels <- whitelist_df$plot_label[match_idx]
  rownames(plot_cor_matrix) <- new_labels
  colnames(plot_cor_matrix) <- new_labels

  # 2. 计算P值矩阵 (用于显著性标记)
  # 定义简易函数计算P值
  get_p_mat <- function(mat) {
    mat <- as.matrix(mat)
    n <- ncol(mat)
    p.mat <- matrix(NA, n, n)
    diag(p.mat) <- 0
  ...
  # 使用完整数据计算P值
  clean_data <- na.omit(model_data[, selected_vars_cor])
  p_matrix <- get_p_mat(clean_data)
  rownames(p_matrix) <- new_labels
  colnames(p_matrix) <- new_labels

  # 3. 绘图 (混合风格：上三角椭圆，下三角数值)
  png("figures/04_collinearity/correlation_heatmap_original.png",
    width = 12000, height = 12000, res = 2400, family = "Arial", bg = "white"
  )

  par(mar = c(2, 2, 2, 2))

  # 第一步：绘制上三角（椭圆 + 显著性X标记）
  corrplot(plot_cor_matrix,
    method = "ellipse",
    type = "upper",
    p.mat = p_matrix,
    sig.level = 0.05,
    insig = "pch", # 不显著打叉
    pch.col = "black",
    pch.cex = 0.4, # 再次缩小叉号 (0.6 -> 0.4)
    tl.pos = "lt", # 标签位置：左+上
    tl.col = "black",
    tl.cex = 0.35, # 再次缩小标签 (0.45 -> 0.35)
    tl.srt = 90, # 标签垂直旋转 (90度) 以彻底避免重叠
    cl.pos = "r", # 图例在右侧
    cl.cex = 0.6,
  ...
  corrplot(plot_cor_matrix,
    ...
    cl.pos = "n", # 不显示图例
    col = "black", # 数值显示为黑色
    number.cex = 0.2, # 再次缩小数值 (0.35 -> 0.2)
  ...
  text(
    x = 1:2, y = var_counts$n_vars + max(var_counts$n_vars) * 0.05,
