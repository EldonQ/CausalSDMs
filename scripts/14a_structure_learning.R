...
S <- cor(X_scaled)
pc_fit <- pcalg::pc(
  suffStat = list(C = S, n = nrow(X_scaled)), indepTest = pcalg::gaussCItest,
...
# 专业出图：完整平均网络（去除孤立点）
set.seed(42)
p_avg_full <- ggraph::ggraph(tg_full, layout = "nicely") +
  ...
  ggrepel::geom_text_repel(aes(x = x, y = y, label = name),
    ...
    box.padding = grid::unit(0.5, "lines"), point.padding = grid::unit(0.3, "lines"),
    segment.color = "grey60", segment.size = 0.25, max.overlaps = Inf, force = 2
  ) +
  labs(title = "Averaged HC-DAG (Non-isolated nodes)", subtitle = paste0("Edges ≥ threshold ", threshold, "; layout=Nicely")) +
  theme_void(base_family = "Arial")

ggsave("figures/14_causal/dag_hc_avg_network_full.png",
  plot = p_avg_full,
  width = 14, height = 10, units = "in", dpi = 1200, bg = "transparent"
)
ggsave("figures/14_causal/dag_hc_avg_network_full.svg",
  plot = p_avg_full,
...
# 专业出图：最大连通分量（LCC）
set.seed(43)
p_avg_lcc <- ggraph::ggraph(tg_lcc, layout = "nicely") +
  ggraph::geom_edge_link(aes(width = strength, alpha = strength),
    arrow = grid::arrow(length = grid::unit(3, "mm"), type = "closed"),
    end_cap = ggraph::circle(3.5, "mm"), colour = "grey30", lineend = "round"
  ) +
  ...
  ggrepel::geom_text_repel(aes(x = x, y = y, label = name),
    size = 3.0, family = "Arial",
    box.padding = grid::unit(0.5, "lines"), point.padding = grid::unit(0.35, "lines"),
    segment.color = "grey60", segment.size = 0.25, max.overlaps = Inf, force = 2
  ) +
  labs(title = "Averaged HC-DAG (Largest Connected Component)", subtitle = paste0("Edges ≥ threshold ", threshold, "; layout=Nicely")) +
  theme_void(base_family = "Arial")

ggsave("figures/14_causal/dag_hc_avg_network_lcc.png",
  plot = p_avg_lcc,
  width = 14, height = 10, units = "in", dpi = 1200, bg = "transparent"
)
ggsave("figures/14_causal/dag_hc_avg_network_lcc.svg",
  plot = p_avg_lcc,
  width = 14, height = 10, units = "in", bg = "transparent"
)
...
p_pc <- ggraph(g_pc_tbl, layout = "sugiyama") +
  ...
  ggrepel::geom_text_repel(aes(x = x, y = y, label = name),
    ...
    box.padding = grid::unit(0.4, "lines"), point.padding = grid::unit(0.2, "lines"),
    max.overlaps = Inf
  ) +
  labs(title = "PC-DAG") +
  theme_void(base_family = "Arial")

ggsave("figures/14_causal/dag_pc.png", plot = p_pc, width = 8, height = 6, units = "in", dpi = 1200, bg = "transparent")
...
if (requireNamespace("Rgraphviz", quietly = TRUE)) {
  ...
  bnlearn::strength.plot(avg_hc,
    strength = boot_hc, threshold = threshold,
    shape = "circle", layout = "dot"
  )
  dev.off()
  png("figures/14_causal/dag_hc.png", width = 8, height = 6, units = "in", res = 1200, type = "cairo-png", family = "Arial", bg = "transparent")
  bnlearn::strength.plot(avg_hc,
    strength = boot_hc, threshold = threshold,
    shape = "circle", layout = "dot"
  )
  ...
  p_hc <- ggraph(g_hc_tbl, layout = "sugiyama") +
    ...
    theme_void(base_family = "Arial")
  ggsave("figures/14_causal/dag_hc.png", plot = p_hc, width = 8, height = 6, units = "in", dpi = 1200, bg = "transparent")
...
ggsave("figures/14_causal/edge_stability.png", plot = p_bar, width = 8, height = 6, units = "in", dpi = 1200, bg = "transparent")
