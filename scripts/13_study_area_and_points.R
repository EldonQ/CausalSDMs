...
# 统一可视化工具（Nature风格、Arial、2400dpi、viridis配色）
...
p_species <- ggplot() +
  ...
  geom_point(
    data = species_data, aes(x = lon, y = lat),
...
p_background <- ggplot() +
  ...
  # 背景点适度缩小点大小
  geom_point(
    data = background_data, aes(x = lon, y = lat),
...
p_combined <- ggplot() +
  ...
  labs(
    title = "Species Presence and Background Points",
    x = "Longitude (\u00B0E)", y = "Latitude (\u00B0N)"
  ) +
  viz_theme_nature(base_size = 9) +
  # 去掉坐标轴与背景网格线
