################################################################################
# Fig 3: Causal Enhancement — Ablation-Focused (4-panel)
#
...
#
# Requires: all_results_v3.csv, all_screening_v3.csv
# Run: setwd("E:/CausalSDMs"); source("scripts/case2/plot/fig3_performance_ablation.R")
################################################################################
...
# (c) Variable reduction vs CAST/MLP AUC ratio
# ══════════════════════════════════════════════════════════════
# Get CAST and MLP AUC per species
cast_mlp <- d %>%
...
# CAST screening: variable reduction
...
# Get n_vars used by CAST from results
cast_nvars <- d %>%
    filter(model == "CAST") %>%
    select(region, species, n_vars)

var_red <- cast_mlp %>%
    inner_join(cast_screened, by = c("region", "species")) %>%
    inner_join(cast_nvars, by = c("region", "species")) %>%
...
fig3 <- (pa | pb) / (pc | pd) +
    plot_annotation(
        title = "Fig 3  Causal Enhancement: Predictive Parity with Parsimony",
        theme = theme(plot.title = element_text(face = "bold", size = 14, hjust = 0.5))
    )

ggsave(file.path(fig_dir, "fig3_performance_ablation.png"),
    fig3,
    width = 14, height = 10, dpi = 300, bg = "white"
)
ggsave(file.path(fig_dir, "fig3_performance_ablation.svg"),
    fig3,
    width = 14, height = 10, bg = "white"
)
cat("Saved fig3_performance_ablation.png/.svg\n")
