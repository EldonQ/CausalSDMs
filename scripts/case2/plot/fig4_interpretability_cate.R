################################################################################
# Fig 4: Ecological Interpretability + CATE Spatial Map (4-panel)
# (a) RF importance rank vs |ATE| rank slope chart
# (b) Spearman rho distribution across species
# (c)(d) CATE spatial heatmap (SWI swi10, 2 key variables)
#
# Requires: all_screening_v3.csv, all_ate_results_v3.csv, disdat package
# Run: setwd("E:/CausalSDMs"); source("scripts/case2/plot/fig4_interpretability_cate.R")
################################################################################
...
# ---- Load data ----
screen <- read.csv("output/case2/all_screening_v3.csv", stringsAsFactors = FALSE)
ate <- read.csv("output/case2/all_ate_results_v3.csv", stringsAsFactors = FALSE)

# ══════════════════════════════════════════════════════════════
# (a) Slope chart: RF importance rank vs |ATE| rank (SWI region)
# ══════════════════════════════════════════════════════════════
# Per species: rank variables by RF importance and by |ATE|
showcase_region <- "SWI"

rank_data <- screen %>%
    filter(region == showcase_region) %>%
    left_join(
...
pa <- ggplot(slope_dat) +
    ...
    labs(
        ...
        x = "", y = "Mean rank across species (1 = most important)"
    ) +
    theme_pub()
...
# ══════════════════════════════════════════════════════════════
# (c)(d) CATE spatial heatmap: SWI swi10
# ══════════════════════════════════════════════════════════════
# Load disdat ENV data for spatial grid + coordinates
if (requireNamespace("disdat", quietly = TRUE)) {
    library(disdat)

    target_sp <- "swi10"
    target_region <- "SWI"

    # Get environmental grid with coordinates
    env_grid <- disdat::disEnv(target_region)
    coord_cols <- c("x", "y")
    meta <- c("siteid", "x", "y")
    env_vars <- setdiff(names(env_grid), meta)
    ...
    sp_ate <- ate %>%
        ...
        filter(region == target_region, species == target_sp, significant == TRUE) %>%
    ...
    if (nrow(sp_ate) >= 2) {
        ...
        # Simple CATE proxy: predict treatment effect heterogeneity
        # using the environmental gradient as a proxy for CATE
        # (full grf CATE would require 04_cate_analysis.R)
        make_cate_panel <- function(env_grid, var_name, ate_coef, panel_label) {
            ...
            plot_df <- env_grid %>%
                select(x, y, all_of(var_name)) %>%
                drop_na() %>%
                mutate(
                    var_scaled = scale(!!sym(var_name))[, 1],
                    cate_proxy = var_scaled * ate_coef
                )

            ggplot(plot_df, aes(x = x, y = y, color = cate_proxy)) +
                ...
                labs(
                    ...
                    subtitle = sprintf("ATE = %.3f | %s", ate_coef, target_sp),
    ...
    pc <- ggplot() +
        theme_void() +
        labs(title = "(c) disdat package required")
    pd <- ggplot() +
        theme_void() +
        labs(title = "(d) disdat package required")
}
...
fig4 <- (pa | pb) / (pc | pd) +
    plot_annotation(
        title = "Fig 4  Ecological Interpretability & Causal Effect Heterogeneity",
        theme = theme(plot.title = element_text(face = "bold", size = 14, hjust = 0.5))
    )

ggsave(file.path(fig_dir, "fig4_interpretability_cate.png"),
    fig4,
    width = 14, height = 11, dpi = 300, bg = "white"
)
ggsave(file.path(fig_dir, "fig4_interpretability_cate.svg"),
    fig4,
    width = 14, height = 11, bg = "white"
)
cat("Saved fig4_interpretability_cate.png/.svg\n")
