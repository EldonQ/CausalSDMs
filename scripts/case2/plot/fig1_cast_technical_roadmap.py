#!/usr/bin/env python3
"""
Fig 1: CAST technical roadmap (main paper).
Copied from 04_cast_technical_roadmap.py; outputs to figures/case2/.
Run from project root: python scripts/case2/plot/fig1_cast_technical_roadmap.py
"""
...
# Project root: script lives in scripts/case2/plot/
SCRIPT_DIR = os.path.dirname(os.path.abspath(__file__))
PROJECT_ROOT = os.path.normpath(os.path.join(SCRIPT_DIR, '..', '..', '..'))
FIG_DIR = os.path.join(PROJECT_ROOT, 'figures', 'case2', 'plot')
...
def main():
    ...
    ax.text(CX + 0.2, mid_v, 'VIF filtering', ha='left', fontsize=8.5, color=GY)
    ...
    ax.text(cate_cx, s2y + 0.48, 'CATE (Spatial)', ha='center', fontsize=11, fontweight='bold', color=BK)
    ax.text(cate_cx, s2y + 0.14, 'Causal Forest  ·  2000 trees', ha='center', fontsize=8, color=GY)
    draw_heatmap(ax, cate_cx, s2y - 0.42, s=0.88)
    ax.text(CX, s2y - 1.05, 'Global ATE per variable  +  Spatial CATE per location',
            ha='center', fontsize=9, color=BK)
    ...
    ax.text(LX + 1.75, s3y + 1.33, 'Adaptive Screening  +  CI-MLP', ha='left', fontsize=13, color=BK)
    crit_y  = s3y + 0.58
    ...
    fig.savefig(os.path.join(FIG_DIR, 'fig1_cast_technical_roadmap.png'), dpi=1200,
                bbox_inches='tight', facecolor='white', edgecolor='none')
    print('[OK] fig1_cast_technical_roadmap.png')
    fig.savefig(os.path.join(FIG_DIR, 'fig1_cast_technical_roadmap.svg'), format='svg',
                bbox_inches='tight', facecolor='white', edgecolor='none')
    print('[OK] fig1_cast_technical_roadmap.svg')
    plt.close()
