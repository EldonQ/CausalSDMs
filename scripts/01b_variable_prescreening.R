...
if (file.exists(override_path)) {
  ...
  cat("启用目标变量模式：仅分析清单中的47个变量，并将附加元数据（如绘图标签）。\n\n")
  target_vars_df <- read.csv(override_path, stringsAsFactors = FALSE)
}
...
for (var_group in names(variable_config)) {
  ...
  # 优化：如果定义了目标变量，且该组没有任何变量在清单中，则直接跳过整个文件
  if (!is.null(target_vars_df)) {
    if (!any(config$band_names %in% target_vars_df$variable)) {
      cat("\n  ℹ 跳过组: ", var_group, " (", config$file, ") - 无目标变量\n", sep = "")
      next
    }
  }
  ...
  tryCatch(
    {
      ...
      for (i in 1:config$total_bands) {
        ...
        # 如果定义了目标变量，且当前变量不在清单中，则跳过
        if (!is.null(target_vars_df) && !(band_name %in% target_vars_df$variable)) {
          next
        }
...
# 如果存在目标变量清单，合并元数据（如绘图标签）
if (!is.null(target_vars_df)) {
  # 合并元数据
  qualified_vars <- qualified_vars %>%
    left_join(target_vars_df %>% select(variable, plot_label, description_en, unit), by = "variable")

  # 确保只保留清单中的变量
  qualified_vars <- qualified_vars %>%
    filter(variable %in% target_vars_df$variable)
...
# 同时也保存一份到 variable_lists 供备份
if (!dir.exists("output/variable_lists")) dir.create("output/variable_lists", recursive = TRUE)
write.csv(qualified_vars, "output/variable_lists/selected_variables_47.csv", row.names = FALSE)
...
# PNG版本（2400dpi，4.8x3.6英寸 -> 扩大到 5x4英寸以容纳边距，即 12000x9600）
png("figures/01b_variable_prescreening/category_summary.png",
  width = 12000, height = 9600, res = 2400, type = "cairo", bg = "transparent"
)
par(mar = c(7, 6, 3, 2), family = "Arial") # 调整边距并使用Arial
barplot(t(category_matrix),
  beside = TRUE,
  col = c("#2E7D32", "#D32F2F"),
