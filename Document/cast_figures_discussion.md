...
[02_multi_species_experiment.R](file:///e:/CausalSDMs/scripts/case2/02_multi_species_experiment.R) 已修复两处问题：
1. **断点续跑去重**：加载 checkpoint CSV 后，对每个 data.frame 按自然主键 `distinct()` 去重
2. **汇总统计 `pivot_wider`**：第 983 行补上 `values_fn = max`

```diff:02_multi_species_experiment.R
################################################################################
...
if (file.exists(file.path(CHECKPOINT_DIR, "all_results_v3.csv"))) {
    ...
    done_count <- all_results %>%
        ...
        filter(n >= MIN_MODELS_PER_SPECIES)
    done_pairs <- paste(done_count$region, done_count$species, sep = "___")
    cat(sprintf("  [Resume] Loaded checkpoint: %d species already done, %d result rows (after dedup).\n",
        nrow(done_count), nrow(all_results)))
} else {
    done_pairs <- character(0)
}
...
## 一、关于 Fig 3 "跨物种因果一致性"的质疑——你说得对

### 你的疑问

> 不同区域的不同物种能有DAG一致性？跨物种ATE方向一致性？？

### 我的反思

你说得完全对，**这张图在科学逻辑上站不住脚**。原因：

1. **不同区域有不同的环境变量**  
   - SWI 用 `bcc`, `calc`, `ddeg`, `nutri`, `precyy` 等瑞士特有变量
   - AWT 用 `bc01`–`bc33` (bioclim) + `slope`, `topo`, `tri`
   - SA 用 `sabio1`–`sabio18`
   - 变量名完全不同，**跨区域 DAG 边频率热图没有可比性**

2. **同一区域不同物种的 DAG 一致性也很弱**  
   - 不同物种对环境的响应机制不同，温度驱动鸟类分布 ≠ 温度驱动植物分布
   - 如果你发现跨物种 DAG 非常一致，那反而说明 DAG 没有学到物种特异的因果结构，只是在学变量间的**统计共线性**

3. **CAST 是方法学文章，不是生态学发现文章**  
   - 方法学文章不需要证明"所有物种的因果结构一样"
   - CAST 要证明的是：**工具在不同数据上都能稳定运行、给出合理结果**

### 替代方案

> [!IMPORTANT]
> **删除原 Fig 3（跨物种因果一致性）**，替换为更符合方法学文章定位的内容——见下文修订方案。
...
| # | 科学问题 | 方法学文章的回答方式 |
|---|---------|-------------------|
| Q1 | 传统 SDM 忽略变量间因果结构，这有什么问题？ | 展示 CAST 发现了因果结构（DAG），指出一些高相关变量实际是非因果的 |
| Q2 | 因果推断能否用于变量筛选？ | 展示 CAST 的自适应筛选机制：筛掉了哪些、保留了哪些 |
| Q3 | 因果增强特征是否对预测有帮助？ | 消融实验（MLP → MLP_ATE → CAST），即使增益小也有正当性 |
| Q4 | CAST 在不同数据集上是否鲁棒？ | 6 区域 × 多物种的大规模实验，工具包"即插即用" |
| Q5 | CAST 的输出是否具有生态可解释性？ | 单物种 showcase + CATE 空间异质性图 |

### 方法学文章的 narrative 逻辑

```
问题: SDM 忽略因果 → 变量选择不透明、模型不可解释
     ↓
方法: CAST pipeline（DAG → ATE → screening → CI-MLP）
     ↓
验证: 6 区域 49 物种大规模实验
     ↓
结论: CAST 工具包能自动发现因果结构、筛选驱动变量、
     保持竞争力预测的同时提供可解释性
```

---

## 三、CATE 空间可解释性地图——可行性分析

### disdat 数据中有坐标吗？

**有！** 但当前 [00_data_preparation.R](file:///E:/CausalSDMs/scripts/case2/00_data_preparation.R) 在第 114 行将 `x`, `y` 列入了 `meta_cols` 并排除了：
...
所以训练/测试数据 CSV 中**没有保存坐标**。但我们可以回溯 disdat 原始数据获取：

| 数据集 | 坐标列 | 用途 |
|--------|--------|------|
| `disPo(region)` | `x`, `y` | 训练点（Presence-only）坐标 |
| `disBg(region)` | `x`, `y` | 背景点坐标 |
| `disEnv(region)` | `x`, `y` | 环境网格点坐标（可做**墙面地图**） |
| `disPa(region)` | `x`, `y` | 测试点坐标 |

各区域坐标系不同：
- **SA**: 经纬度 WGS84
- **SWI**: 瑞士网格 EPSG:21781
- **AWT**: UTM zone 55 (EPSG:28355)
- **NSW/NZ**: 各自的投影坐标系

### CATE 地图的实现思路

> [!TIP]
> 可以用 `disEnv()` 获取环境网格点 + 坐标，然后对每个网格点预测 CATE，生成空间热力图。

具体步骤：
1. 对选定的代表性物种（如 SWI 的 swi10），加载 `disEnv("SWI")` 获取网格 `(x, y)` + 环境变量
2. 对目标因果变量（如 `ddeg`），将其分为"高/低"两组，计算每个网格点的 CATE（即处理效应的空间分布）
3. 用 `ggplot2 + geom_tile/geom_point` 绘制空间热力图
4. 展示"温度对物种分布的因果效应在空间上并非均匀"

> [!WARNING]
> CATE 需要 Causal Forest（`grf` 包）或类似方法来估计条件异质性效应。当前 [02_multi_species_experiment.R](file:///e:/CausalSDMs/scripts/case2/02_multi_species_experiment.R) 只估计了 **ATE**（平均效应），没有 CATE。需要新增一个专门的 CATE 分析脚本。

### 结论

**CATE 空间地图在技术上完全可行**，但需要：
1. 在出图脚本中从 `disdat` 获取 `disEnv()` 网格坐标数据
2. 新增 CATE 估计逻辑（用 `grf::causal_forest`）
3. 选 1-2 个代表性物种（如 SWI 的 swi10）做展示

---

## 四、三个补充分析维度

### 维度 1：变量减少比例 vs AUC 保持

**科学意义**：证明 CAST 的因果筛选能"精简不损失"——用更少的变量达到相当的预测性能。

**数据来源**：已有！`all_screening_v3.csv` + `all_results_v3.csv`

**图表设计**：

| 子图 | 内容 |
|------|------|
| (a) 散点图 | **x** = 变量减少比例 `(n_vars_after_vif - n_selected) / n_vars_after_vif`，**y** = CAST AUC / RF AUC（AUC 保持率）。每个点是一个物种。理想区域：右上角（减少多、性能保持） |
| (b) 分布图 | 各模型使用的变量数 vs AUC 的 violin/box 对比 |

**预期 narrative**：
> "CAST 平均筛掉了 X% 的变量，但 AUC 仅下降 Y%（甚至持平），说明被筛除的变量确实是非因果的冗余变量。"

---

### 维度 2：传统相关性排名 vs 因果排名

**科学意义**：CAST 找到了"不同的东西"——传统方法认为重要的变量（RF importance / correlation），和因果分析认为重要的变量（ATE大小 / DAG out-degree），排名可能截然不同。

**数据来源**：已有！`all_screening_v3.csv` 中有 `importance`（RF）和 `coef`（ATE）

**图表设计**：

| 子图 | 内容 |
|------|------|
| (a) Slope chart | 选 1 个代表区域，左侧 = RF importance rank，右侧 = |ATE| rank。线条交叉越多 = 两种排名差异越大 |
| (b) Rank difference scatter | **x** = RF rank，**y** = ATE rank，每个点一个变量。偏离对角线越远 = 差异越大 |
| (c) 统计量 | Spearman ρ (RF rank vs ATE rank) 在各区域的分布——ρ 越低说明因果和相关性看到的是不同维度 |

**预期 narrative**：
> "RF importance（相关性驱动）和 ATE（因果驱动）的变量重要性排名仅有中等相关（平均 Spearman ρ = 0.4-0.6），说明因果分析确实揭示了相关性分析看不到的结构——某些高相关但非因果的变量被降权，某些低相关但因果显著的变量被提升。"

---

### 维度 3：跨区域鲁棒性

**科学意义**：CAST 作为工具包，在不同生态系统中表现是否稳定？

**数据来源**：已有！`all_results_v3.csv`

**图表设计**：

| 子图 | 内容 |
|------|------|
| (a) 箱线图 | 每个模型在 6 个区域间 AUC 的 **标准差/IQR** 对比——CAST 的 IQR 是否比其他方法更小？ |
| (b) Rank stability | 每个区域中各模型的平均排名 ± SD——CAST 排名是否更稳定（SD 更小）？ |
| (c) Heatmap | 每个 (region, model) 的平均 AUC 热图，颜色编码 |

**预期 narrative**：
> "虽然 CAST 在单区域中不一定排第一，但其跨区域的性能方差更小（rank SD = X vs baseline's Y），说明因果信息的注入提供了更稳定的预测基础。"

> [!NOTE]
> 这一点需要从数据中验证。如果实际上 CAST 的方差并不比别人小，那这个维度就不能作为主张。从现有结果看，CAST 在 SWI（0.80）和 AWT（0.62）之间差距确实不小，但这可能是数据本身的差异而非方法的问题。
...
> [!IMPORTANT]
> 正文图只保留 **4 张**，每张必须服务于明确的科学问题。

| 图号 | 标题 | 科学问题 | 子图 |
|------|------|---------|------|
| **Fig 1** | CAST 方法论框架 | 方法总览 | 技术路线图（Python 生成） |
| **Fig 2** | 单物种因果发现 showcase | Q1+Q2: CAST 找到了什么？ | (a) DAG 网络 (b) ATE forest plot (c) 自适应筛选得分 |
| **Fig 3** | 预测性能 + 因果增强消融 | Q3: 因果信息有用吗？ | (a) 全模型 AUC 对比 violin (b) MLP→MLP_ATE→CAST 消融 waterfall (c) 变量减少 vs AUC 保持 |
| **Fig 4** | 可解释性 + CATE 空间地图 | Q5: 输出有何生态意义？ | (a) RF rank vs ATE rank slope chart (b) CATE 空间热力图 (1-2 个物种) |
...
|------|------|
| Fig S1 | 各区域 DAG 密度分布 |
| Fig S2 | ATE 效应方向（按区域分面） |
| Fig S3 | 跨区域鲁棒性（rank stability） |
| Fig S4 | CAST vs baselines 逐区域详细对比 |
| Fig S5 | DAG density vs CAST gain （诊断图） |

---

## 六、等待你的决定

1. **Fig 1 框架图**：保持 Python 生成（已有），还是从头重画？
2. **Fig 2 单物种 showcase**：用哪个代表性物种？SWI 的 swi10（数据最丰富），还是你有偏好？
3. **Fig 3 中要不要做消融？** 当前 MLP_ATE 的数据不完整（AWT 只有 2 个、CAN 只有 1 个成功），是否需要先重跑？
4. **Fig 4 CATE 地图**：是否确认要做？需要新增 `grf` 分析脚本
5. **三个补充维度**：哪些纳入正文，哪些放补充材料？
6. **你觉得还缺什么？** 比如 Nemenyi 检验、paired t-test 等统计检验表？
