# CAST × disdat：数据策略分析 + 最终出图方案

## 一、disdat 数据集介绍与当前使用策略

### disdat 是什么？

disdat（Distributional Species Data）是 Elith et al. (2020) 发布的 **SDM 基准数据集**，提供了 6 个全球代表性区域的物种分布数据，用于系统比较不同 SDM 方法。

| 区域 | 代码 | 分类群 | 坐标系 |
|------|------|--------|--------|
| Australian Wet Tropics | AWT | 鸟类、植物 | UTM zone 55 (EPSG:28355) |
| Ontario, Canada | CAN | 鸟类 | UTM |
| New South Wales | NSW | 植物 | UTM zone 56 |
| New Zealand | NZ | 植物 | NZ Map Grid |
| South America | SA | 植物 | WGS84 经纬度 |
| Switzerland | SWI | 植物 | Swiss Grid (EPSG:21781) |

### disdat 提供的 4 种数据

| 数据集 | R 函数 | 内容 | 列结构 |
|--------|--------|------|--------|
| **PO** (Presence-Only) | `disPo(region)` | 训练：物种出现记录 | `siteid`, `spid`, **`x`, `y`**, `occ`, `group`, + 环境变量列 |
| **BG** (Background) | `disBg(region)` | 训练：10,000 个背景点 | `siteid`, **`x`, `y`**, + 环境变量列 |
| **PA** (Presence-Absence) | `disPa(region)` | 测试：独立 PA 调查 | `siteid`, **`x`, `y`**, + 每个物种的 0/1 列 |
| **ENV** (Environment) | `disEnv(region)` | 环境网格点（可做地图预测） | `siteid`, **`x`, `y`**, + 环境变量列 |

> [!IMPORTANT]
> **所有数据集都有 `x`, `y` 坐标列！** 这是 CATE 空间地图的基础。

### 各区域的环境变量（完全不同！）

| 区域 | 环境变量 |
|------|---------|
| AWT | bc01-bc33 (bioclim), slope, topo, tri |
| CAN | 类似 bioclim 系列 |
| NSW | 区域特有变量 |
| NZ | 区域特有变量 |
| SA | sabio1-sabio18 |
...
### 当前 [00_data_preparation.R](file:///E:/CausalSDMs/scripts/case2/00_data_preparation.R) 的处理策略

```r
# 第 114 行：将 x, y 视为元数据列并排除
meta_cols <- c("siteid", "spid", "x", "y", "occ", "group")
env_cols <- setdiff(names(po), meta_cols)

# 第 161-164 行：训练数据只保留环境变量 + presence
sp_po <- po %>%
    filter(spid == sp) %>%
    select(all_of(env_cols)) %>%  # 丢弃了 x, y 坐标！
    mutate(presence = 1)

# 第 168-172 行：测试数据同样丢弃了坐标
test <- pa_full %>%
    select(all_of(c(sp, env_cols))) %>%  # 没有 x, y
    rename(presence = !!sp) %>%
    drop_na()
```
...
| 方面 | 状态 | 说明 |
|------|------|------|
| 基本结构 | OK | PO+BG 训练, PA 测试, 符合 disdat benchmark 标准做法 |
| 环境变量 | OK | 取 PO 和 BG 的交集列作为 env_cols |
| 物种筛选 | 合理 | min_po >= 200 过滤掉样本过少的物种 |
| PA 合并 | OK | 多 group 区域(AWT, NSW)逐 group 加载后合并 |
| **坐标保存** | 缺失 | x, y 被排除,导致无法做空间分析 |
| **环境网格 ENV** | 未充分使用 | 虽然加载了 disEnv(),但主要只用于 PA 合并 |
...
| # | 问题 | 修复 |
|---|------|------|
| 1 | MIN_MODELS_PER_SPECIES = 6 导致 MLP_ATE 永远被跳过 | 改为 **7** (全部模型齐全才跳过) |
| 2 | 不完整物种的旧数据残留 | 加入 **清理逻辑**: 模型不齐的物种所有数据清除, 完整重跑 |
| 3 | MLP_ATE 错误被静默吞掉 | 加入 **日志输出** |
| 4 | GAM s(x, k=5) 在变量唯一值 < 5 时崩溃 | 改为 **自适应 k**: k = min(5, n_unique - 1) |

### 预期效果

重跑 02 后:
- AWT 的 MLP_ATE 从 2 变为 **8**
- CAN 的 MLP_ATE 从 1 变为 **10**
- NSW/NZ/SWI 的 GAM 从 0 变为 有值
- **消融实验数据将完整**
...
train_sdm <- function(sdm_name, X_tr_raw, y_tr_raw, X_te_raw, gam_vars = NULL) {
    switch(sdm_name,
        ...
        "GAM" = {
            gv <- if (!is.null(gam_vars)) gam_vars else names(X_tr_raw)
            # 自适应 k: 确保 k 不超过该变量的唯一值数量
            train_for_gam <- cbind(presence = y_tr_raw, X_tr_raw[, gv, drop = FALSE])
            smooth_terms <- sapply(gv, function(v) {
                n_unique <- length(unique(train_for_gam[[v]]))
                k_val <- max(3L, min(5L, n_unique - 1L))
                sprintf("s(%s, k=%d)", v, k_val)
            })
            gf <- as.formula(paste("presence ~", paste(smooth_terms, collapse = " + ")))
            gm <- mgcv::gam(gf,
                data = train_for_gam,
                family = binomial(), method = "REML"
            )
...
cat("======================================================================\n")
cat(sprintf("  CAST Multi-Region Experiment: %d regions\n", length(REGIONS)))
cat("  Models: CAST, MLP_ATE, MLP, RF, Maxent, BRT, GAM\n")
cat("======================================================================\n\n")
...
CHECKPOINT_DIR <- "output/case2"
ALL_MODEL_NAMES <- c("CAST", "MLP_ATE", "MLP", "RF", "Maxent", "BRT", "GAM")
MIN_MODELS_PER_SPECIES <- length(ALL_MODEL_NAMES) # 全部 7 个模型齐全才跳过
...
if (file.exists(file.path(CHECKPOINT_DIR, "all_results_v3.csv"))) {
    ...
    if (nrow(incomplete) > 0) {
        ...
        cat(sprintf("  [Resume] Cleaning %d incomplete species for re-run: %s\n",
...
### Fig 2: 单物种因果发现 showcase (SWI swi10)
- 科学问题: CAST 工具包能发现什么?
- (a) 共识 DAG 网络图
- (b) ATE forest plot (效应大小 + 95% CI)
- (c) 自适应筛选得分 (堆叠柱状图)

### Fig 3: 预测性能 + 因果增强消融 + 变量精简
- 科学问题: 因果信息有用吗? 变量精简效果如何?
- (a) 全模型 AUC violin 对比 (7 个模型)
- (b) 消融 waterfall: MLP, +ATE, +DAG = CAST
- (c) 变量减少比例 vs AUC 保持率 散点图
- (d) 按区域排名 bump chart

### Fig 4: 因果 vs 相关性解释 + CATE 空间地图
- 科学问题: CAST 输出有何生态意义?
- (a) RF importance rank vs |ATE| rank slope chart
- (b) Spearman rho 分布
- (c)(d) CATE 空间热力图 (SWI swi10)

### Fig 5: 跨区域鲁棒性
- 科学问题: CAST 在不同生态系统中是否稳定?
- (a) 各模型跨区域 AUC 的 IQR/SD 对比
- (b) (region, model) AUC 热力图
- (c) Mean rank +/- SD 对比图
...
| 脚本 | 状态 | 内容 |
|------|------|------|
| 02_multi_species_experiment.R | 已修改 | 断点续跑 + GAM + MLP_ATE 修复 |
| plot/fig3_performance_and_ablation.R | 新建 | 合并性能对比 + 消融 + 变量精简 |
| plot/fig4_interpretability_and_cate.R | 新建 | RF vs ATE + CATE 空间地图 |
| plot/fig5_cross_region_robustness.R | 新建 | 跨区域鲁棒性分析 |
| scripts/case2/04_cate_analysis.R | 新建 | CATE 估计 (grf::causal_forest) |

---

## 五、下一步

1. 你先重跑 02_multi_species_experiment.R (补齐 MLP_ATE + GAM)
2. 同时我来写 04_cate_analysis.R + 新出图脚本
